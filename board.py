...
class Board:
    ...
    def initialize_pieces(self):
        ...
        self.pieces.clear()
        piece_radius = int(self.hex_radius * 0.7) # Piece size relative to hex

        rows_per_player = (self.side_length // 2) + 1 # Adjust as needed for balance

        for q, r in self.hex_coords:
            # Player 1 (Top - assuming negative r is 'up')
            if r < -(self.side_length - 1) + rows_per_player :
                 piece = Piece(q, r, PLAYER1, piece_radius)
                 self.pieces[(q, r)] = piece

            # Player 2 (Bottom - assuming positive r is 'down')
            elif r > (self.side_length - 1) - rows_per_player:
                 piece = Piece(q, r, PLAYER2, piece_radius)
                 self.pieces[(q, r)] = piece

        # Initialize pixel positions (will be animated)
    ...
    def initialize_tile_fall_animation(self):
        ...
        start_y_offset = -self.screen_height // 2 # Start above the screen

        # Stagger the fall slightly
        delay_factor = 0
        sorted_coords = sorted(self.hex_coords, key=lambda coord: coord[1]) # Fall roughly row by row

        for q, r in sorted_coords:
            target_x, target_y = hex_to_pixel(q, r, self.hex_radius, self.center_x, self.center_y)
            start_y = target_y + start_y_offset #- delay_factor * 5 # Add small delay
            self.tile_fall_data.append({'q': q, 'r': r, 'y': start_y, 'target_y': target_y})
            # delay_factor += 1
    ...
    def update_tile_fall_animation(self):
        """Update falling tile positions. Returns True if animation is ongoing."""
        animating = False
        if not self.tile_fall_data:
            return False # No tiles left to animate

        # Animate the first tile in the list
        tile_data = self.tile_fall_data[0]
        tile_data['y'] += FALL_SPEED_TILE
        if tile_data['y'] >= tile_data['target_y']:
            tile_data['y'] = tile_data['target_y']
            self.tiles_drawn.add((tile_data['q'], tile_data['r']))
            self.tile_fall_data.pop(0) # Remove finished tile
            animating = len(self.tile_fall_data) > 0 # Still animating if list not empty
        else:
             animating = True # This tile is still falling

        return animating


    def draw_board(self, surface, theme):
        """Draws the hexagonal board tiles."""
        color_index = 0
        # Draw tiles that have finished falling
        ...
        if self.tile_fall_data:
            ...
            temp_center_y = tile_data['y']

            # Need to recalculate corners based on temporary center
            temp_corners = []
            for i in range(6):
                angle_deg = 60 * i
                ...
                temp_corners.append(
                    (temp_center_x + self.hex_radius * math.cos(angle_rad),
                     temp_center_y + self.hex_radius * math.sin(angle_rad))
                )

            color = theme["board_light"] if (q + r) % 2 == 0 else theme["board_dark"]
            pygame.draw.polygon(surface, color, temp_corners)
            pygame.draw.polygon(surface, theme["text"], temp_corners, 1) # Outline
